...
namespace DTcms.DAL
{
    ...
    /// 数据库访问层
    /// </summary>
    public partial class dt_plugin_feedback
    {
        private string column;
        private string databaseprefix; 
        public dt_plugin_feedback(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "dt_plugin_feedback] where id=@id", parameters);
        }
        ...
        public bool Exists(string title)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "dt_plugin_feedback] where title=@title", parameters);
        }
        ...
        public string GetTitle(int id)
        {
            ...
            parameters[0].Value = id;
            object obj = DbHelperSQL.GetSingle("select title from [" + databaseprefix + "dt_plugin_feedback] where id=@id", parameters);
            if (null != obj)
            {
                return obj.ToString();
            }
        ...
        public int GetCount(string strWhere)
        {
            ...
            strSql.Append("select count(*) as H from [" + databaseprefix + "dt_plugin_feedback]");
            if ("" != strWhere.Trim())
            {
        ...
        /// 增加一条数据
        /// </summary>
        /// <param name="model">Model.dt_plugin_feedback</param>
        /// <returns>ID</returns>
        public int Add(Model.dt_plugin_feedback model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into [" + databaseprefix + "dt_plugin_feedback](");
        ...
        {
            DbHelperSQL.ExecuteSql("update [" + databaseprefix + "dt_plugin_feedback] set " + strValue + " where id=" + id);
        }
        #endregion
        ...
        /// <param name="model">Model.dt_plugin_feedback</param>
        /// <returns>True or False</returns>
        public bool Update(Model.dt_plugin_feedback model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [" + databaseprefix + "dt_plugin_feedback] set ");
        ...
        public bool Delete(int id)
        {
            ...
            parameters[0].Value = id;
            int rows = DbHelperSQL.ExecuteSql("delete from [" + databaseprefix + "dt_plugin_feedback] where id=@id", parameters);
            if (rows > 0)
            {
                return true;
            }
        ...
        /// </summary>
        /// <param name="id">ID号</param>
        /// <returns>Model.dt_plugin_feedback</returns>
        public Model.dt_plugin_feedback GetModel(int id)
        {
            SqlParameter[] parameters = {
            		new SqlParameter("@id", SqlDbType.Int,4)
            };
            parameters[0].Value = id;
            DataSet ds = DbHelperSQL.Query("select " + this.column + " from [" + databaseprefix + "dt_plugin_feedback] where id=@id", parameters);
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(this.column + " from [" + databaseprefix + "dt_plugin_feedback]");
            if ("" != strWhere.Trim())
            {
              strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [" + databaseprefix + "dt_plugin_feedback]");
            if ("" != strWhere.Trim())
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// <returns>Model.dt_plugin_feedback</returns>
        private Model.dt_plugin_feedback DataRowToModel(DataRow row)
        {
            Model.dt_plugin_feedback model = new Model.dt_plugin_feedback();
            if (row != null)
            {
