...
namespace DTcms.DAL
{
    ...
    public partial class dt_manager
    {
        ...
        public dt_manager(string _databaseprefix)
        {
            ...
            this.column = "id,role_id,role_type,user_name,password,salt,real_name,telephone,email,is_lock,add_time";
        }
        ...
        public bool Exists(int id)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "dt_manager] where id=@id", parameters);
        }
        /// <summary>
        /// 返回数据总数
        /// </summary>
        ...
        public int GetCount(string strWhere)
        {
            ...
            strSql.Append("select count(*) as H from [" + databaseprefix + "dt_manager]");
            if ("" != strWhere.Trim())
            {
        ...
        /// 增加一条数据
        /// </summary>
        /// <param name="model">Model.dt_manager</param>
        /// <returns>ID</returns>
        public int Add(Model.dt_manager model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into [" + databaseprefix + "dt_manager](");
        ...
        {
            DbHelperSQL.ExecuteSql("update [" + databaseprefix + "dt_manager] set " + strValue + " where id=" + id);
        }
        #endregion
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [" + databaseprefix + "dt_manager] set ");
        ...
        public bool Delete(int id)
        {
            ...
            parameters[0].Value = id;
            int rows = DbHelperSQL.ExecuteSql("delete from [" + databaseprefix + "dt_manager] where id=@id", parameters);
            if (rows > 0)
            {
                return true;
            }
        ...
        /// </summary>
        /// <param name="id">ID号</param>
        /// <returns>Model.dt_manager</returns>
        public Model.dt_manager GetModel(int id)
        {
            ...
            parameters[0].Value = id;
            DataSet ds = DbHelperSQL.Query("select " + this.column + " from [" + databaseprefix + "dt_manager] where id=@id", parameters);
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(this.column + " from [" + databaseprefix + "dt_manager]");
            if ("" != strWhere.Trim())
            {
              strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [" + databaseprefix + "dt_manager]");
            if ("" != strWhere.Trim())
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// <returns>Model.dt_manager</returns>
        private Model.dt_manager DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
            	...
            	if (null != row["real_name"])
            	{
            		model.real_name = row["real_name"].ToString();
            	}
            	if (null != row["telephone"])
            	{
