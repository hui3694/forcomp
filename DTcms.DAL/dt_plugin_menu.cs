...
namespace DTcms.DAL
{
    ...
    /// 数据库访问层
    /// </summary>
    public partial class dt_plugin_menu
    {
        private string column;
        private string databaseprefix; 
        public dt_plugin_menu(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "dt_plugin_menu] where id=@id", parameters);
        }
        ...
        public bool Exists(string title)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "dt_plugin_menu] where title=@title", parameters);
        }
        ...
        public string GetTitle(int id)
        {
            ...
            parameters[0].Value = id;
            object obj = DbHelperSQL.GetSingle("select title from [" + databaseprefix + "dt_plugin_menu] where id=@id", parameters);
            if (null != obj)
            {
                return obj.ToString();
            }
        ...
        public int GetCount(string strWhere)
        {
            ...
            strSql.Append("select count(*) as H from [" + databaseprefix + "dt_plugin_menu]");
            if ("" != strWhere.Trim())
            {
        ...
        public int Add(Model.dt_plugin_menu model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    {
                        StringBuilder strSql = new StringBuilder();
                        strSql.Append("insert into [" + databaseprefix + "dt_plugin_menu](");
            			...
            			{
            				Model.dt_plugin_menu model2 = GetModel(conn, trans, model.parent_id); //带事务
            			...
            			//修改节点列表和深度
            			DbHelperSQL.ExecuteSql(conn, trans, "update [" + databaseprefix + "dt_plugin_menu] set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
            			trans.Commit();
                    }
                    catch
                    {
        ...
        {
            DbHelperSQL.ExecuteSql("update [" + databaseprefix + "dt_plugin_menu] set " + strValue + " where id=" + id);
        }
        #endregion
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
            			...
            			{
            				//查找旧数据
            				Model.dt_plugin_menu oldModel = GetModel(model.id);
            				//查找旧父节点数据
            				string class_list = "," + model.parent_id + ",";
            				int class_layer = 1;
            				if (oldModel.parent_id > 0)
            				{
            					Model.dt_plugin_menu oldParentModel = GetModel(conn, trans, oldModel.parent_id); //带事务
            					class_list = oldParentModel.class_list + model.parent_id + ",";
            					class_layer = oldParentModel.class_layer + 1;
            				}
            				//先提升选中的父节点
            				DbHelperSQL.ExecuteSql(conn, trans, "update [" + databaseprefix + "dt_plugin_menu] set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
            			...
            			StringBuilder strSql = new StringBuilder();
            			strSql.Append("update [" + databaseprefix + "dt_plugin_menu] set ");
        ...
        public bool Delete(int id)
        {
            ...
            parameters[0].Value = id;
            int rows = DbHelperSQL.ExecuteSql("delete from [" + databaseprefix + "dt_plugin_menu] where id=@id", parameters);
            if (rows > 0)
            {
                return true;
            }
        ...
        public Model.dt_plugin_menu GetModel(int id)
        {
            ...
            parameters[0].Value = id;
            DataSet ds = DbHelperSQL.Query("select " + this.column + " from [" + databaseprefix + "dt_plugin_menu] where id=@id", parameters);
        ...
        /// <param name="id">ID号</param>
        public Model.dt_plugin_menu GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            SqlParameter[] parameters = {
        					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            DataSet ds = DbHelperSQL.Query(conn, trans, "select " + this.column + " from [" + databaseprefix + "dt_plugin_menu] where id=@id", parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
            }
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(this.column + " from [" + databaseprefix + "dt_plugin_menu]");
            if ("" != strWhere.Trim())
            {
              strSql.Append(" where " + strWhere);
            }
        ...
        /// <returns>Model.dt_plugin_menu</returns>
        private Model.dt_plugin_menu DataRowToModel(DataRow row)
        {
            Model.dt_plugin_menu model = new Model.dt_plugin_menu();
            if (row != null)
            {
        ...
        {
            //查找父节点信息
            Model.dt_plugin_menu model = GetModel(conn, trans, parent_id);
            if (null != model)
            {
        		//查找子节点
        		string strSql = "select id from [" + databaseprefix + "dt_plugin_menu] where parent_id=" + parent_id;
        		...
        		foreach (DataRow dr in ds.Tables[0].Rows)
        		{
        			...
        			DbHelperSQL.ExecuteSql(conn, trans, "update [" + databaseprefix + "dt_plugin_menu] set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务
        			//调用自身迭代
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [" + databaseprefix + "dt_plugin_menu] where class_list like '%," + id + ",%' and id=" + parent_id);
