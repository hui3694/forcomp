...
namespace DTcms.Web.admin.order
{
    public partial class order_confirm : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_confirm", DTEnums.ActionEnum.View.ToString()); //检查权限
                //确认权限
                if (!ChkAuthority("order_config", DTEnums.ActionEnum.Confirm.ToString()))
                {
        ...
        {
            ChkAdminLevel("order_confirm", DTEnums.ActionEnum.Confirm.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
