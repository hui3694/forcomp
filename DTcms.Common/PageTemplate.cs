...
namespace DTcms.Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        /// <returns></returns>
        private static string ConvertTags(int nest, string channelName, string pageSize, string isCache, string cachePath, string sitePath, string tempPath, string skinName, string inputStr, string _type, int _site_id)
        {
            string strReturn = "";
            ...
            foreach (Match m in r[0].Matches(strTemplate))
            {
                ...
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, "templates", m.Groups[3].ToString(), m.Groups[4].ToString(), string.Empty, string.Empty, string.Empty, channelName, pageSize, isCache, cachePath, _type, _site_id, nest + 1) + "\r\n");
                }
                else
                {
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "\r\n" + GetTemplate(sitePath, tempPath, skinName, m.Groups[4].ToString(), string.Empty, string.Empty, string.Empty, channelName, pageSize, isCache, cachePath, _type, _site_id, nest + 1) + "\r\n");
                }
            }
            #endregion
            ...
            foreach (Match m in r[24].Matches(strTemplate))
            {
                IsCodeLine = true;
                strTemplate = strTemplate.Replace(m.Groups[0].ToString(),
                              string.Format("\r\n\ttemplateBuilder.Append(Utils.ReplaceStr({0}));", m.Groups[2]).Replace("\\\"", "\""));
            }
            #endregion
