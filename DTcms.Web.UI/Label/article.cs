...
namespace DTcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        /// </summary>
        /// <param name="channel_name">频道名称</param>
        /// <param name="category_id">类别名称</param>
        ...
        /// <param name="orderby">排序</param>
        /// <returns></returns>
        public DataTable get_similar_article_list(string channel_name, int category_id, string field, int top, string strwhere, string orderby)
        {
            DataTable dt = new DataTable();
            if (!string.IsNullOrEmpty(channel_name))
            {
                if (!string.IsNullOrEmpty(field))
                {
                    ...
                    {
                        strwhere = "id in (" + field + ")";
                    }
                    else
                    {
                        strwhere += " and id in (" + field + ")";
                    }
                }
                dt = new BLL.article().GetList(channel_name, category_id, top, strwhere, orderby).Tables[0];
            }
