...
namespace DTcms.Web.UI.Page
{
    public partial class search : Web.UI.BasePage
    {
        ...
        /// 查询索引数据
        /// </summary>
        protected DataTable get_lucene_list(int _pagesize, out int _totalcount, int _site_id)
        {
            //创建一个DataTable
            string link_url = null;
            DataTable dt = null;
            DataTable oldData = get_lucene_list(_pagesize, this.page, _site_id, this.channel, 0, this.keyword, out _totalcount);
            dt = oldData.Clone();

            if (oldData.Rows.Count > 0)
            {
                dt.Columns.Add("link_url", Type.GetType("System.String"));
                //dt.Columns.Remove("is_top");
                //dt.Columns.Remove("is_red");
                //dt.Columns.Remove("is_hot");
                //dt.Columns.Remove("market_price");
                //dt.Columns.Remove("sell_price");
                //dt.Columns.Remove("sub_title");
                dt.Columns["zhaiyao"].ColumnName = "remark";
                ...
                foreach (DataRow dr in oldData.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(link_url))
                    {
                        ...
                        row["remark"] = dr["zhaiyao"];
                        row["img_url"] = dr["img_url"];
                        row["link_url"] = link_url;
                        row["add_time"] = dr["add_time"];
                        row["update_time"] = dr["update_time"];
                        row["tags"] = dr["tags"];
                        row["titlehighlight"] = dr["titlehighlight"];
                        row["zhaiyaohighlight"] = dr["zhaiyaohighlight"];
                        dt.Rows.Add(row);
                    }
                }
            }
