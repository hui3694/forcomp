...
namespace DTcms.Web.Plugin.Forum.admin
{
    ...
    //------------------------------------------------------
    public partial class post_subtable_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == DTEnums.ActionEnum.Edit.ToString())
            {
                ...
                if (!new BLL.Forum_PostSubTable().Exists(this.id))
                {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("plugin_forum_postsubtable", DTEnums.ActionEnum.Edit.ToString()); //检查权限
                ...
                JscriptMsg("修改信息成功！", "post_subtable_list.aspx");
            }
            else //添加
            {
                ChkAdminLevel("plugin_forum_postsubtable", DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加信息成功！", "post_subtable_list.aspx");
            }
        }
    }
}