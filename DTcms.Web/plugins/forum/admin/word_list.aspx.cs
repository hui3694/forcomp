...
{	
    ...
    //------------------------------------------------------
    public partial class word_list : Web.UI.ManagePage
	{
		//页面属性
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("Forum_Word_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            ChkAdminLevel("plugin_forum_word", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.Forum_Word bll = new BLL.Forum_Word();
            ...
            AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除dt_Forum_Word" 
            	+ sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("Word_list.aspx", "keywords={0}", this.keywords));
        }
    }
}