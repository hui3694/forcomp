...
namespace DTcms.Web.Plugin.Forum.admin
{
    ...
    //------------------------------------------------------
    public partial class attachment_list : Web.UI.ManagePage
	{
		//页面属性
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("" + CombSqlTxt(keywords), "");//添加where子句和order子句
            }
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("Forum_Attachment_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("Forum_Attachment_list.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("Forum_Attachment_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("Forum_Attachment_list.aspx", "keywords={0}", this.keywords));
        }
