...
namespace DTcms.DAL
{
    ...
    /// 数据库访问层
    /// </summary>
    public partial class plugin_images_class
    {
        private string column;
        private string databaseprefix; 
        public plugin_images_class(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "plugin_images_class] where id=@id", parameters);
        }
        ...
        public bool Exists(string title)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from [" + databaseprefix + "plugin_images_class] where title=@title", parameters);
        }
        ...
        public string GetTitle(int id)
        {
            ...
            parameters[0].Value = id;
            object obj = DbHelperSQL.GetSingle("select title from [" + databaseprefix + "plugin_images_class] where id=@id", parameters);
            if (null != obj)
            {
                return obj.ToString();
            }
        ...
        public int GetCount(string strWhere)
        {
            ...
            strSql.Append("select count(*) as H from [" + databaseprefix + "plugin_images_class]");
            if ("" != strWhere.Trim())
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [" + databaseprefix + "plugin_images_class] set " + strValue);
        ...
        public bool Delete(int id)
        {
            ...
            parameters[0].Value = id;
            int rows = DbHelperSQL.ExecuteSql("delete from [" + databaseprefix + "plugin_images_class] where is_sys=0 and id=@id", parameters);
            if (rows > 0)
            {
                DbHelperSQL.ExecuteSql("delete from [" + databaseprefix + "plugin_images] where class_id=@id", parameters);
                return true;
            }
            else
            {
        ...
        public Model.plugin_images_class GetModel(int id)
        {
            SqlParameter[] parameters = {
            		new SqlParameter("@id", SqlDbType.Int,4)
            };
            parameters[0].Value = id;
            DataSet ds = DbHelperSQL.Query("select " + this.column + " from [" + databaseprefix + "plugin_images_class] where id=@id", parameters);
        ...
        /// <returns></returns>
        public Model.plugin_images_class GetModel(string call_index)
        {
            SqlParameter[] parameters = {
            		new SqlParameter("@call_index", SqlDbType.NVarChar,50)
            };
            parameters[0].Value = call_index;
            DataSet ds = DbHelperSQL.Query("select top 1 " + this.column + " from [" + databaseprefix + "plugin_images_class] where call_index=@call_index", parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
            }
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(this.column + " from [" + databaseprefix + "plugin_images_class]");
            if ("" != strWhere.Trim())
            {
              strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [" + databaseprefix + "plugin_images_class]");
            if ("" != strWhere.Trim())
            {
                strSql.Append(" where " + strWhere);
            }
