...
namespace DTcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        protected Model.channel chlmodel; 
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //频道信息
            this.chlmodel = new BLL.channel().GetModel(this.channel_id);
            if (null == this.chlmodel)
            {
                JscriptMsg("传输参数不正确！", "back");
                return;
            }
            this.channel_name = this.chlmodel.name; //取得频道名称
            ...
            if (!Page.IsPostBack)
            {
                ...
                //添加编辑权限
                if (action == DTEnums.ActionEnum.Copy.ToString() && !ChkAuthority("channel_" + this.channel_name + "_list", DTEnums.ActionEnum.Add.ToString()))
                {
                    this.btnSubmit.Visible = false;
                }
                else if (action != DTEnums.ActionEnum.Copy.ToString() && !ChkAuthority("channel_" + this.channel_name + "_list", this.action))
                {
                    this.btnSubmit.Visible = false;
                }
        ...
        private string AutoGetFirstImage(string content)
        {
            ...
            if (m.Success)
            {
                ...
                if (!string.IsNullOrEmpty(imgUri))
                {
                    ...
                    if (this.chlmodel.width > 0 && this.chlmodel.height > 0)
                    {
                        string path = Utils.GetMapPath(imgUri);
                        Thumbnail.MakeThumbnailImage(path, path, this.chlmodel.width, this.chlmodel.height, this.siteConfig.thumbnailmode);
                    }
                    else if (this.siteConfig.thumbnailwidth > 0 && this.siteConfig.thumbnailheight > 0)
                    {
                        string path = Utils.GetMapPath(imgUri);
                        Thumbnail.MakeThumbnailImage(path, path, this.siteConfig.thumbnailwidth, this.siteConfig.thumbnailheight, this.siteConfig.thumbnailmode);
                    }
                    return newUrl;
                }
            }
        ...
        private bool DoAdd()
        {
            ...
            //封面图片
            if (string.IsNullOrEmpty(model.img_url) && string.IsNullOrEmpty(txtImgUrl.Text.Trim()) && autoImg.Checked == true)
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改信息成功！", "article_list.aspx" + backUrl);
            }
            else //添加
            {
                ...
                JscriptMsg("添加信息成功！", "article_list.aspx" + backUrl);
            }
        }
